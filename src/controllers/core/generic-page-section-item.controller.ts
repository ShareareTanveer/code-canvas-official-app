...
import sectionItemService from '../../services/core/generic-page-section-item.service';
import {
...
const getById: IController = async (req, res) => {
  try {
    ...
    const data = await sectionItemService.getById(id);
...
const list: IController = async (req, res) => {
  try {
    const data = await sectionItemService.list();
    return ApiResponse.result(res, data, httpStatusCodes.OK);
  } catch (e) {
...
const create: IController = async (req, res) => {
  try {
    ...
    const data = await sectionItemService.create(params);
...
const update: IController = async (req, res) => {
  try {
    ...
    const data = await sectionItemService.update(id, params);
    return ApiResponse.result(res, data, httpStatusCodes.OK);
  } catch (e) {
    return ApiResponse.error(res, httpStatusCodes.BAD_REQUEST, e.message);
  }
...
const remove: IController = async (req, res) => {
  try {
    const id: number = parseInt(req.params.id, 10);
    await sectionItemService.remove(id);
