...
import service from '../../services/product/productCategory.service';
import ApiUtility from '../../utilities/api.utility';
import { IBaseQueryParams } from 'common.interface';
import { ICreateProductCategory, IUpdateProductCategory } from 'product/productCategory';
...
const create: IController = async (req, res) => {
  try {
    ...
    const params: ICreateProductCategory = {
      name: req.body.name,
...
const update: IController = async (req, res) => {
  try {
    const imageLocalFile = req.file?.path;
    const id: number = parseInt(req.params.id, 10);
    const params: IUpdateProductCategory = {
      name: req.body.name,
      image: imageLocalFile,
      color: req.body.color,
      subtitle: req.body.subtitle,
